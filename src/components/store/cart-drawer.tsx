          ...
          {!items.length ? (
            ...
            <>
                  ...
                  <div className="flex gap-4" key={item.variantId}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">{item.variantTitle}</p>
                       <p className="mt-1 font-semibold">{new Intl.NumberFormat("pt-BR", { style: "currency", currency: item.price.currencyCode }).format(Number(item.price.amount))}</p>
              ...
              <div className="border-t pt-5">
                 <div className="mb-4 flex justify-between text-lg font-semibold"><span>Total</span><span>{new Intl.NumberFormat("pt-BR", { style: "currency", currency: items[0]?.price.currencyCode ?? "BRL" }).format(total)}</span></div>
                <Button className="w-full" size="lg" disabled={!checkoutUrl || isLoading || isSyncing} onClick={() => { if (checkoutUrl) { const currency = items[0]?.price.currencyCode; trackCommerceEvent("begin_checkout", { value: total, ...(currency ? { currency } : {}), quantity: count }); window.open(checkoutUrl, "_blank"); } setOpen(false); }}>
